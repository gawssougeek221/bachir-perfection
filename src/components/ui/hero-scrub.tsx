...
export function HeroScrub() {
  ...
  return (
    <section
      ref={containerRef}
      className="relative w-full h-screen min-h-[600px] overflow-hidden bg-[#080709]"
    >
      {FRAME_MAP.map((num, i) => (
        <img
          key={num}
      ...
      <div ref={overlayRef} className="absolute inset-0 z-[1]" style={{ background: "linear-gradient(105deg, rgba(8,7,9,0.92) 0%, rgba(8,7,9,0.6) 30%, rgba(8,7,9,0.3) 55%, rgba(8,7,9,0.15) 100%)" }} />

      <div ref={textRef} className="absolute inset-0 z-[2] flex items-center pointer-events-none" style={{ willChange: "transform, opacity" }}>
        <div className="px-6 md:px-12 lg:px-20 xl:px-28 pointer-events-auto max-w-xl lg:max-w-2xl">
          ...
          <h1 className="text-3xl md:text-5xl lg:text-6xl xl:text-7xl font-bold tracking-tight text-white leading-[1.05]">
            <span className="block">PERFECTION IS<br />NOT REPAIR.</span>
            <span className="block mt-2 text-[#D4AF37]" style={{ textShadow: "0 0 40px rgba(212,175,55,0.2)" }}>
              IT IS REBIRTH.
            </span>
          </h1>
          ...
          <div className="flex flex-col sm:flex-row gap-3 mt-8 w-fit">
            <a href="#transformations" className="inline-flex items-center gap-2 bg-[#D4AF37] text-[#111] px-7 py-3.5 rounded-full text-[11px] md:text-sm tracking-wider uppercase font-semibold hover:shadow-[0_0_40px_rgba(212,175,55,0.3)] transition-shadow duration-500">
            ...
            <a href="https://wa.me/221XXXXXXXX" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 bg-white/[0.07] backdrop-blur-md text-white/80 px-7 py-3.5 rounded-full text-[11px] md:text-sm tracking-wider uppercase font-medium border border-white/[0.12] hover:border-[#D4AF37]/50 hover:text-[#D4AF37] transition-all duration-500">
      ...
      <style jsx>{`@keyframes scrollPulse { 0% { transform: translateY(-100%); } 100% { transform: translateY(200%); } }`}</style>
    </section>
  );
}
